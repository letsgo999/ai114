...
export function inferCategory(keywords: string[]): string {
  ...
  const categoryKeywords: Record<string, string[]> = {
    ...
    '웹크롤링': ['크롤링', '스크래핑', '웹스크래핑', '데이터수집', '웹자동화', '모니터링'],  // 신규
    '음악생성': ['음악', '뮤직', '작곡', 'BGM', '배경음악', '노래', '보컬', '멜로디'],  // 신규
    '다목적': ['파이썬', 'Python', '스크립트', '브라우저', 'AI어시스턴트']  // 신규
  };
...
const CATEGORY_HINT_MAPPING: Record<string, string[]> = {
  ...
  '개발': ['개발', '업무자동화', '다목적'],
  '리서치': ['리서치', '데이터분석'],
  '웹크롤링': ['데이터분석', '업무자동화', '다목적', '리서치'],  // 웹 크롤링 → Colab, Comet 등
  '음악생성': ['음악생성'],  // 음악 생성
  '다목적': ['다목적', '문서작성', '데이터분석']
};
...
const CATEGORY_PREFERRED_TOOLS: Record<string, string[]> = {
  ...
  '웹크롤링': ['Google Colab', 'Perplexity Comet', 'Gemini in Chrome', 'Listly', 'ChatGPT'],  // 웹 크롤링 전용
  '음악생성': ['MiniMax Music', 'Suno AI', 'Udio', 'AIVA'],  // 음악 생성 전용
