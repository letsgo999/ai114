...
export function inferCategory(keywords: string[]): string {
  ...
  const categoryKeywords: Record<string, string[]> = {
    ...
    '데이터분석': ['데이터', '분석', '통계', '차트', '그래프', '시각화', '엑셀', '성과', 'Pandas', 'Matplotlib'],
    ...
    '음성생성': ['TTS', '음성합성', '나레이션', '더빙', 'AI성우', 'AI보이스', '음성복제', '보이스클론'],  // TTS 신규
    '자막생성': ['자막', 'STT', '음성인식', '전사', '자동자막', '자막생성', '번역자막'],  // STT/자막 신규
...
const CATEGORY_HINT_MAPPING: Record<string, string[]> = {
  ...
  '개발': ['개발', '업무자동화', '다목적'],
  '리서치': ['리서치', '데이터분석'],
  '웹크롤링': ['데이터분석', '업무자동화', '다목적', '리서치'],
  '음악생성': ['음악생성'],
  '음성생성': ['음성생성'],  // TTS 카테고리 신규
  '자막생성': ['영상편집', '회의'],  // STT/자막 → Vrew, Descript 등
  '다목적': ['다목적', '문서작성', '데이터분석']
};
...
const CATEGORY_PREFERRED_TOOLS: Record<string, string[]> = {
  ...
  '데이터분석': ['Google Colab', 'ChatGPT', 'Claude', 'Julius AI', 'Perplexity Comet', 'Gemini in Chrome'],
  '마케팅': ['Gemini Gems', 'ChatGPT', 'Canva', 'Copy.ai'],
  '업무자동화': ['Zapier', 'Make', 'n8n', 'Microsoft Power Automate', 'Google Colab'],
  '웹크롤링': ['Google Colab', 'Perplexity Comet', 'Gemini in Chrome', 'Listly', 'ChatGPT'],
  '음악생성': ['MiniMax Music', 'Suno AI', 'Udio', 'AIVA'],
  '음성생성': ['ElevenLabs', 'Supertone', 'Gemini TTS', 'Qwen3 TTS', 'Google AI Studio TTS'],  // TTS 전용 신규
  '자막생성': ['Vrew', 'Descript', 'Clova Note', 'CapCut'],  // STT/자막 전용 신규
  '다목적': ['Google Colab', 'ChatGPT', 'Claude', 'Gemini Gems', 'Perplexity Comet']
};
